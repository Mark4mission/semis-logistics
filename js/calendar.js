...
(() => {
  ...
  const LS_FIRED = "semisl:firedRem";
  ...
  const ui = () => { try { return JSON.parse(localStorage.getItem("semisl:ui")) || {}; } catch (e) { return {}; } };
  const setUi = (patch) => localStorage.setItem("semisl:ui", JSON.stringify(Object.assign(ui(), patch)));
  ...
  function gcalForm() {
    ...
    openModal(`
      ...
      <div class="form-row"><label>구글 캘린더 ID</label>
        <input id="g-calid" value="${esc(cfg.calendarId || "")}" placeholder="예: xxxx@gmail.com (공개 캘린더 ID)"></div>
