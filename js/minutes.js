...
(() => {
  ...
  function seedFolders() {
    return [
      ...
      { id: "mf-team", seq: 2, icon: "🏢", name: "인천화물팀 안전보안회의", desc: "팀 안전보안 회의·TBM 협의", place: "", chair: "" },
      { id: "mf-agency", seq: 3, icon: "🏛", name: "유관기관 협의", desc: "국토부·공항공사·세관·경찰 등 대외 협의", place: "", chair: "" },
      { id: "mf-partner", seq: 4, icon: "🤝", name: "조업사·협력사 회의", desc: "조업사·협력업체 안전보안 간담회", place: "", chair: "" },
      { id: "mf-edu", seq: 5, icon: "🎓", name: "교육·훈련 강평", desc: "안전보안 교육·비상훈련 사후 강평", place: "", chair: "" },
      { id: "mf-incident", seq: 6, icon: "🚨", name: "사고·이슈 대응", desc: "안전·보안 사고 및 현안 대응 회의", place: "", chair: "" },
  ...
  const orgPresets = () => {
    ...
    ["안전보안파트", "인천화물팀", "항공보안파트", "운항품질팀", "조업사", "협력업체", "국토교통부", "인천국제공항공사", "인천세관", "공항경찰"]
      .forEach(o => s.add(o));
    return Array.from(s);
  };
    ...
    return `<div class="mn-signbox">
      ...
      <div class="mn-signbox-body">
        ...
        <ol class="mn-signbox-steps">
          ...
          <li>QR이 안 될 때는 <b>시스템 접속 주소</b>로 들어가 로그인 창에 암호 <b class="cn-signcode-code">${esc(code)}</b> 입력.</li>
      ...
      box.innerHTML = `
        ...
        <div class="mn-sv-grid">
          <div class="mn-sv-left">
            ...
            <div class="mn-sv-codelabel">QR이 안 될 때 · 시스템 로그인 창에 입력</div>
     ...
     <div class="cn-form mn-form">
      ...
      <fieldset class="cn-fs"><legend>📋 회의 정보</legend>
        ...
        <div class="form-row"><label>제목 *</label>
          <input id="mn-title" value="${esc(base.title || "")}" maxlength="90" placeholder="예: 안전보안파트 제12차 정례회의"></div>
        <div class="form-grid">
  ...
  async function printHTML(html, label) {
    try {
      ...
      if (window.SemisFileAuth) html = await SemisFileAuth.signHtml(html);   // 별도 문서라 화면 자동 변환이 닿지 않는다
...
<title>${esc(x.title || "회의록")}</title><style>${PRINT_CSS}</style></head><body>
  <div class="doc-head">
    ...
    <div class="sub">에어제타 인천화물팀 안전보안파트 · ${esc(folderName(x.folder))}${x.no ? " · 제" + esc(String(x.no)) + "차" : ""}${x.status === "draft" ? " · [초안]" : ""}</div>
    <div class="meta">
...
<title>참석 서명 QR — ${esc(title)}</title><style>
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  <div class="box">
    ...
    <div class="code-wrap">
      <div class="code-label">QR이 안 될 때 — 시스템 접속 후 로그인 창에 아래 번호 입력</div>
  ...
  function personModal(minuteId, idx) {
    ...
    openModal(`
      ...
      <div class="form-row"><label>소속 *</label>
        <input id="mn-sp-org" value="${esc(init.org)}" maxlength="40" list="mn-sp-orgs" placeholder="예: 안전보안파트">
    ...
    if (saveOnly) saveOnly.onclick = () => {
      ...
      if (!person) return;
      Promise.resolve(saveSignEntry(minuteId, idx, person, null)).then(ok => {
        if (ok === false) return;
    ...
    $("#mn-sp-go").onclick = () => {
      ...
      openSignPad(person, (val) => {
        Promise.resolve(saveSignEntry(minuteId, idx, person, val)).then(ok => {
          if (ok === false) return;
  ...
  /* 서명 세션 — 서버가 이 회의 한 건만 고친다(회의록 전체를 보내지 않음) */
  async function signerSave(minuteId, idx, person, sign) {
    const m = all().find(c => c.id === minuteId);
    const expect = (idx >= 0 && m && Array.isArray(m.attendees) && m.attendees[idx]) ? m.attendees[idx].name : null;
    let d;
    try { d = await SeMIS.signSubmit(idx, expect, person, sign === null || sign === undefined ? null : (sign || "")); }
  ...
  function saveSignEntry(minuteId, idx, person, sign) {
    if (SeMIS.user && SeMIS.user.role === "signer") return signerSave(minuteId, idx, person, sign);
    const m = all().find(c => c.id === minuteId);
    if (!m) return false;
