...
(() => {
  ...
  function rowHTML(en, i) {
    return `<tr>
      ...
      <td><b>${esc(en.title)}</b>${en.note ? `<div style="font-size:.76rem;color:var(--text-3)">${esc(en.note)}</div>` : ""}</td>
      <td style="font-size:.86rem">${en.account ? `${esc(en.account)} <button class="ct-copy" data-vc-acc="${i}" title="계정 복사">📋</button>` : "-"}</td>
      <td style="font-size:.86rem;white-space:nowrap">${en.pw ? `<span class="v-mask" data-vp-span="${i}">••••••••</span>
      ...
      <td><button class="btn btn-ghost btn-sm no-print" data-ve-edit="${esc(en.id)}">✎</button></td>
    </tr>`;
  }
  function unlockedBody() {
    const items = sortedItems();
    if (!items.length) {
      return `<div class="empty">${query ? "검색 결과가 없습니다." : '등록된 항목이 없습니다. "+ 항목 추가"로 시작하세요.'}</div>`;
    }
