...
(() => {
  ...
  const SCOPES = {
    sec: { route: "reg-sec", title: "항공보안 규정", icon: "📘",
           desc: "화물 보안 관련 법령 · 자체 보안규정",
           orgPh: "예: CYB0xx / 국토교통부 / 화물서비스팀" },
    safety: { route: "reg-safety", title: "안전관리 규정", icon: "🦺",
           desc: "표준업무절차 · 교범 · 품질/안전 절차서",
           orgPh: "예: CYB027 / 화물서비스팀" },
    dg: { route: "reg-dg", title: "위험물(DG) 기준", icon: "☢️",
           desc: "위험물 교범 · 취급 기준",
           orgPh: "예: CYA002 / 화물서비스팀" }
  };
  const PDF_MAX = 25 * 1024 * 1024;
  const LANGS = ["", "국문", "영문", "국·영문"];
  ...
  const canSeeIdeas = () => SeMIS.roleRank() >= 2;   // 내부 검토 자료: 화물팀 관리자 이상

  let query = { sec: "", safety: "", dg: "" };
  ...
  function regForm(scope, id) {
    ...
    openModal(`
      ...
      <div class="form-row"><label>규정명 (제목)</label>
        <input id="rg-title" value="${esc(x ? x.title : "")}" maxlength="120" placeholder="예: 화물 표준업무절차 (CSOP)"></div>
      <div class="form-grid">
        <div class="form-row"><label>버전 (Revision)</label>
          <input id="rg-rev" value="${esc(x ? x.rev || "" : "")}" maxlength="30" placeholder="예: Rev.03 / 제4차 개정"></div>
        <div class="form-row"><label>제·개정일자</label>
          <input type="date" id="rg-date" value="${esc(x ? x.date || "" : "")}"></div>
      </div>
      <div class="form-grid">
        <div class="form-row"><label>관리번호 / 주관부서</label>
          <input id="rg-org" value="${esc(x ? x.org || "" : "")}" maxlength="80" placeholder="${esc(sc.orgPh)}"></div>
        <div class="form-row"><label>언어</label>
          <select id="rg-lang">${LANGS.map(l =>
            `<option value="${esc(l)}" ${(x ? x.lang || "" : "") === l ? "selected" : ""}>${esc(l || "(지정 안 함)")}</option>`).join("")}</select></div>
      </div>
      <div class="form-row"><label>규정 링크 (URL)</label>
        ...
        <div class="form-hint">링크 또는 PDF 중 하나 이상</div></div>
    ...
    $("#rg-save").onclick = () => {
      ...
      const rec = {
        ...
        org: $("#rg-org").value.trim(), lang: $("#rg-lang").value, linkUrl,
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row"><label>상태</label>
          <select id="ri-status">${IDEA_STATUS.map(st =>
            `<option ${(n ? n.status : "검토중") === st ? "selected" : ""}>${st}</option>`).join("")}</select></div>
      </div>
